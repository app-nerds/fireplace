...
docker-tag: ## Builds a docker image and tags a release. It is then pushed up to Docker. GITHUB_TOKEN must be defined as an environment variable. Usage: make USERNAME="username" APP_NAME="appname" APP_PATH="./cmd/appname" docker-tag
	...
	docker build --platform linux/arm64 --build-arg GITHUB_TOKEN=${GITHUB_TOKEN} --build-arg APP_NAME=fireplace-viewer --build-arg APP_PATH=./cmd/fireplace-viewer --tag appnerds/fireplace-viewer:linux-arm64-${VERSION} . && docker push appnerds/fireplace-viewer:linux-arm64-${VERSION}
